...
data_file = "/cubric/scratch/c1557187/stirring/RESULTS/ERF/clockwise_anticlocwise_all_sensors_mean.npy"
...
for subset in sensor_groupings.keys():
    ...
    long.plot(long_times, easy_mean, linewidth=1, color=col["clockwise"], label="Clockwise movement")
    long.fill_between(
        long_times,
        easy_mean - easy_sem,
        easy_mean + easy_sem,
    ...
    long.plot(long_times, difficult_mean, linewidth=1, color=col["anti-clockwise"], label="Anti-clockwise movement")
    long.fill_between(
        long_times,
        difficult_mean - difficult_sem,
        difficult_mean + difficult_sem,
        color=col["anti-clockwise"], 
        alpha=0.2, 
        linewidth=0
    )
    long.legend(loc=4)
    ...
    # data proc
    easy = np.array(data["Engage"][selection]["clockwise"]) * 1e14
    easy = np.mean(easy[:, ch_ix, :], axis=1)
    easy_mean = np.mean(easy, axis=0)
    easy_sem = sem(easy, axis=0)
    difficult = np.array(data["Engage"][selection]["anti-clockwise"]) * 1e14
    difficult = np.mean(difficult[:, ch_ix, :], axis=1)
    difficult_mean = np.mean(difficult, axis=0)
    difficult_sem = sem(difficult, axis=0)
    ...
    engage.plot(short_times, easy_mean, linewidth=1, color=col["clockwise"], label="Clockwise movement")
    engage.fill_between(
        short_times,
        easy_mean - easy_sem,
        easy_mean + easy_sem,
    ...
    engage.plot(short_times, difficult_mean, linewidth=1, color=col["anti-clockwise"], label="Anti-clockwise movement")
    engage.fill_between(
        short_times,
        difficult_mean - difficult_sem,
        difficult_mean + difficult_sem,
    ...
    # data proc
    easy = np.array(data["Change"][selection]["clockwise"]) * 1e14
    easy = np.mean(easy[:, ch_ix, :], axis=1)
    easy_mean = np.mean(easy, axis=0)
    easy_sem = sem(easy, axis=0)
    difficult = np.array(data["Change"][selection]["anti-clockwise"]) * 1e14
    difficult = np.mean(difficult[:, ch_ix, :], axis=1)
    difficult_mean = np.mean(difficult, axis=0)
    difficult_sem = sem(difficult, axis=0)
    ...
    change.plot(short_times, easy_mean, linewidth=1, color=col["clockwise"], label="Clockwise movement")
    change.fill_between(
        short_times,
        easy_mean - easy_sem,
        easy_mean + easy_sem,
    ...
    change.plot(short_times, difficult_mean, linewidth=1, color=col["anti-clockwise"], label="Anti-clockwise movement")
    change.fill_between(
        short_times,
        difficult_mean - difficult_sem,
        difficult_mean + difficult_sem,
    ...
    png = op.join(output_path, "cva_{}_{}.png".format(selection.lower().replace(" ", "_"), subset.lower().replace(" ", "_")))
    svg = op.join(output_path, "cva_{}_{}.svg".format(selection.lower().replace(" ", "_"), subset.lower().replace(" ", "_")))
    plt.savefig(png)
    plt.savefig(svg)
